...
describe("UserUsername Test Suite", () => {
  ...
  describe(".from()", () => {
    it("should return a Success<UserUsername> instance with the provided id", () => {
      const result = UserUsername.from("any_username");
      const userUsername = result.value;

      expect(result).toBeInstanceOf(Success<UserUsername>);
      expect(userUsername.value).toBe("any_username");
    });
  });
});
